import { asPaidTier } from "./tiers";
import type { SupabaseClient } from "@supabase/supabase-js";
import { isSupabaseConfigured } from "@/lib/supabase/config";
import { createClient } from "@/lib/supabase/server";
import { asProvider } from "./provider";
import { asPeriod } from "./plans";
import { asSubscriptionStatus, type Subscription } from "./subscription";
 ...
 *
 * **Export must never move behind the plan**, so a plan check is the wrong tool
 ...
 *
 * With no Supabase project there are no accounts to check, so a self-hosted
 * copy is unchanged.
 */
