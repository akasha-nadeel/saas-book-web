...
export function PriceCheckPage({
  ...
  const [shelfLabel, setShelfLabel] = useState<string>(
    () => shelfForGenre(book?.genre)?.label ?? "",
  );
  const [ownGenre, setOwnGenre] = useState("");
  const [setting, setSetting] = useState("");
  const [event, setEvent] = useState("");
  /** Whether the writer has asked for the form back after an answer. */
  const [editing, setEditing] = useState(false);

  const [look, setLook] = useState<PriceLook | null>(null);
...
/**
 * Every priced book, cheapest first.
 *
 ...
 * search than any figure could tell them. It is also the table view the plot
 * needs to be readable without colour or a pointer.
 */
function PriceList({ look }: { look: PriceLook }) {
  return (
    <section className="mt-5 rounded-2xl border border-line bg-panel p-5 @2xl:p-6">
      ...
      <ul className="mt-4 divide-y divide-line">
        {look.prices.map((p) => (
          <li
            key={p.book.key}
            /* **`items-center`, not `items-baseline`.** A baseline row lines
               the cover's bottom edge up with the title's baseline and hangs
               the thumbnail below the row. */
            className="flex items-center gap-3 py-2.5"
          >
            {/* `BookCover` draws a spine when the catalogue gave no thumbnail,
                so a book without one is still a book rather than a gap — which
                is what makes a list of thirty read as a shelf. Its `<img>` is
                lazy, which is what keeps thirty third-party requests down to
                the rows actually on screen. */}
            {/* **Squarer than `BookCover`'s own `rounded-lg`**, because a
                printed cover has near-square corners and 8px on a 32px
                thumbnail reads as an app icon rather than a book.

                The `!` is load-bearing: two radius utilities land on one
                element and which one wins is decided by the order Tailwind
                emits them, not the order they are written here, so the plain
                class would be a coin toss that happened to land right. */}
            ...
            <span className="shrink-0 font-semibold text-fg tabular-nums">
              {p.amount === 0 ? "Free" : money(p.amount)}
            </span>
          </li>
        ))}
      </ul>
